...
func (b *Product) GetProducts(c *gin.Context) {
	...
	db := models.GetDBv2().Table("Products").
		Limit(params.PageSize).
		Offset(offset).
		Order("created_at desc")

	if len(params.Guid) != 0 {
		db = db.Where("guid = ?", params.Guid)
	}

	if len(params.SiteID) > 1 {
		db = db.Where("domain(product_url) = ?", params.SiteID)
	}
	if params.Category != "" {
	...
	if params.SearchQuery != "" {
		...
		db = db.Where("title ilike ? or brand = ?", q, params.SearchQuery)
	}
	if params.Featured != "" {
		db = db.Where("featured ilike ?", params.Featured)
	}

	err = db.Scan(&prods).Error
	if err != nil {
		log.WithError(err).Error("could not list products")
		c.AbortWithStatus(http.StatusInternalServerError)
		return
	}

	c.Header("x-total-count", fmt.Sprint(len(prods)))
