...
func crawl() {
	...
	for i := 0; ; i++ {
		...
		// 	err := models.GetDBv2().Clauses(clause.OnConflict{Columns: []clause.Column{{Name: "product_url"}}, UpdateAll: true}).Create(&products).Error
		// 	if err != nil {
		// 		log.WithError(err).Error("could not save products to db")
		// 		return
		// 	}

		// 	log.WithField("size", batchSize).Debug("saved a batch of products")

		// 	totalSaved += len(products)

		// 	// clear products - so we start another batch
		// 	products = products[:0]
		// }
