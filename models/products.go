...
func GetProductsCount(searchQuery string, category string, siteID int64, featured string) (uint64, error) {

	var count struct {
		Count uint64 `gorm:"count"` // TODO: probably not needed like this, fix
	}

	db := GetDBv2().Table("Products p").Select("count(*) AS count")

	if siteID != 0 {
		db = db.Where("p.site_id = ?", siteID)
	}

	if category != "" {
		db = db.Where("p.category = ?", category)
	}

	if featured != "" {
		db = db.Where("p.featured = ?", featured)
	}

	if searchQuery != "" {
		expr := fmt.Sprintf("%%%v%%", searchQuery) // %+str+%
		db = db.Where("p.title LIKE ? OR p.brand LIKE ?", expr, expr)
	}

	err := db.Limit(1).Find(&count).Error
	if err != nil {
		return 0, errors.Wrap(err, "could not retrieve product count")
	}
...
func GetProducts(searchQuery string,
	offset int,
	itemsPerPage int,
	category string,
	siteID int64,
	...
	q := `SELECT id,
		...
		FROM Products `
	//WHERE category = ?
	//AND Products.brand IN (?)`

	if category != "" {
		q += fmt.Sprintf(`where lowerUTF8(category) = '%s'`, category)
	}

	//if searchQuery != "" {
	//	expr := fmt.Sprintf("%%%v%%", searchQuery)
	//	q += fmt.Sprintf(" and lowerUTF8(title) like '%s' or lowerUTF8(brand) like '%s'", expr, expr)
	//}

	//switch sortBy {
	//case 0:
	//	b = b.OrderBy("approved desc")
	//case 1:
	//	b.OrderBy("created_at asc")
	//case 2:
	//	b.OrderBy("created_at desc")
	//default:
	//	return nil, errors.Errorf("unknown sort order")
	//}

	q += ` LIMIT 10 OFFSET 0`

	fmt.Printf("built search query ************ %s", q)
	//q, args, err := b.ToSql()

	//if err != nil {
	//	log.Fatalf("unable to build query %s", err)
	//}
	var prods []Product
	err := GetDBv2().Raw(q).Scan(&prods).Error
...
// Used by Admin
func GetProductsByApprovedStatus(approvedStatus bool, offset int64, itemsPerPage int64, search, category string, siteID int64, sortBy int) ([]Product, error) {
	var ps []Product
	db := GetDBv2().
		Table("Products").
	...
	if search != "" {
		searchQuery := "%" + search + "%"
		db = db.Where("title = ? or brand = ?", searchQuery, searchQuery)
	}
...
// Used by Admin
func GetProductsCountByStatus(approvedStatus bool, search, category string, siteId int64) (count int64, err error) {
	searchQuery := "%" + search + "%"
	err = GetDB().QueryRow(
		`SELECT COUNT(*) FROM (
			SELECT * FROM Products 
			WHERE Products.category = IF(? = '',category, ?)
			AND Products.site_id = IF(? = 0, site_id, ?)
			AND approved = ?
		) AS Product
